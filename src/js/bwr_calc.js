...
var cCalc = (function (window, document) {
  ...
  var calcSelStart,
    calcSelEnd, // variables to keep track of caret position or selection in calc input area
    maxResults = 500,
    history = History(maxResults), // variables for history
    queryCount = 0,
    background = chrome.extension.getBackgroundPage(),
    //chromeyCalcHelperId = "kncknbclhdncdolkfleoeefggfbclgpp"; // dev id
    chromeyCalcHelperId = 'hfgnndipkjcpghbagmmcemdbjfclkcla'; // release id
  var queryMode = {
    all: {
      firstStatus: 'trying js',
      queryTypeByStatus: {
        // Used to decide to query js or Google or give up
        'trying js': 'js',
      ...
      nextStatus: {
        // Used to decide what query to try next if last query failed
    ...
    js: {
      ...
      firstStatus: 'trying js',
      queryTypeByStatus: {
        // Used to decide to query js or Google give up
        'trying js': 'js',
        failed: '',
      },
      nextStatus: {
  ...
  function popOutCalc() {
    ...
    background.calcPopOut = background.open(
      'calc.html',
      'calcPopOut',
      localStorage.popOutWindowInfo || defaultPopOutWindowInfo
    );
  }

  // Store pop-out position and dimensions as a single string that can be passed to window.open()
  function savePopOutWindowInfo() {
    ...
    localStorage.popOutWindowInfo =
      'resizable=yes' + height + width + top + left;
  }
  ...
  function calcInit(currentWindow) {
    ...
    $(function () {
      ...
      $calcInput.keydown(function (e) {
        ...
        if (e.which === 13 && inputVal) {
          ...
          if (inputVal === 'clear') {
            // Clear results
            $calcResults.empty();
          } else if (inputVal.indexOf('useIcon(') == '0') {
            // Change Chromey's toolbar icon
            iconName =
              'icon_' + inputVal.slice(8, inputVal.length - 1) + '.png'; // Strip the 'useIcon(' and ')' from our param here
            // Don't change icon unless it exists (list of possible icons set in background.html)
            if (iconName in background.icons) {
              localStorage.useIcon = iconName;
              chrome.browserAction.setIcon({
                path: background.icons[iconName],
              });
            }
          } else if (inputVal.indexOf('cc(') == '0') {
            // A Chromey Calculator Command
            // Strip the 'cc(' and ')' from our param here. Make an array of arguments
            cmdArgs = inputVal.slice(3, inputVal.length - 1).split(/\s*,\s*/);
            calcCmd[cmdArgs[0]] &&
              calcCmd[cmdArgs[0]].apply({}, cmdArgs.slice(1));
          } else {
            // Continue from previous result (when we can). Looking for: + - * / ^ % (sticking with the basics for now...)
            if (inputVal.match(/^[+\-*\/^%]/)) {
              // operator doesn't need space before it
              inputVal = '@' + inputVal;
            } else if (
              inputVal.match(
                /^plus\s+|^minus\s+|^times\s+|^x\s+|^divided\s+by\s+|^mod\s+|^modulo\s+|^modulus\s+/i
              )
            ) {
              // operator needs space before it
              inputVal = '@ ' + inputVal;
            }
            ...
            calc.findResult(inputVal, function () {
              function updateResultsArea($calcResults, $calcResultsWrapper) {
                ...
                // Scroll to bottom
                $calcResultsWrapper[0].scrollTop =
                  $calcResultsWrapper[0].scrollHeight;
                // Limit number of results to maxResults
                ...
                $results
                  ...
                  .find('.resultLink')
                  .show()
                  .css({ opacity: maxResultLinkOpacity })
                  .animate({ opacity: minResultLinkOpacity }, 2000);
              }
          ...
          setTimeout(function () {
            $calcInput[0].selectionStart = $calcInput[0].selectionEnd =
              $calcInput.val().length;
          }, 0);
        } else if (e.which === 40) {
          // Down arrow
      ...
      $(document).delegate(
        '.outputText, .inputText, .replacedInputText, .errorInputText, .errorOutputText, .inputTextWithVars, .replacedVarAssignmentInputText, .varAssignmentInputText, .varAssignmentOutputText',
        'click',
        function (e) {
          var $this = $(this);
          ...
          if ($(e.target).is('.plotLink')) {
            ...
            // calculate new location for insertion (so results are inserted from left to right)
      ...
      $calcResultsWrapper.scroll(function () {
        $calcInput.focus();
      });
    });
  }
  ...
  function setGoogleQueryUriHead() {
    ...
    if (
      background.helperIsInstalled &&
      getOption('localGoogleOn') &&
      getOption('localGoogleUrl')
    ) {
  ...
  calcStore = (function () {
    ...
    function loadCalcInfo() {
      ...
      // restore calc input value and caret position (or text selection)
    ...
    function storeCalcInfo() {
      ...
      $calcResults
        .find('.resultLink')
        .css({ display: 'block', opacity: minResultLinkOpacity });
  ...
  //			varVal: <value of variable, only if inspecting variable>,
  ...
  calc = (function () {
    ...
    function cleanNumbers(input) {
      return (
        input
          ...
          .replace(
            RegExp('(\\d)\\' + digitGroupSeparator + '(\\d)', 'g'),
            '$1$2'
          )
    ...
    function findResult(input, callback) {
      // Reset result object
      var result = (calc.result = { prevResult: calc.result }), // Used to help construct all the pieces of the result html
      ...
      if (input !== result.origInput && input !== result.varRhExpr) {
        ...
        // Make output undefined substitution contains undefined
    ...
    function calcQuery(input, callback) {
      ...
      // Remove digit separators, set decimal marker to "."
      ...
      function queryCallback(doc) {
        ...
        if (!output.plain) {
          ...
          if (result.status === 'trying google, did you mean') {
            ...
            if (result.varName) {
              ...
              result.correctedInput =
                result.varName + ' = ' + calcQuery.correctedInput;
            } else {
          ...
          result.queryType = queryType;
          result.uri = createQueryUri[queryType](
            input.replace(rxCleanInput, '')
          );
          callback && callback(result);
        }
      }
  ...
  createQueryUri = (function () {
    ...
    function generateInputGoogleQueryUri(input) {
      input = input
        // add multiplication sign between parentheses
        .replace(/\)\s*\(/g, ')*(')
        // add multiplication between digit char and "("
        .replace(/(\d)\s*\(/g, '$1*(')
        // add multiplication between ")" and digit char
        .replace(/\)\s*(\d)/g, ')*$1')
  ...
  extractCalcOutput = (function () {
    ...
    function sanitaryEval(input) {
      var output = null,
        max = Math.pow(10, maxDigits),
        regexInputNotSanitary = /[^+\-*\/%.()\d\s]/;
      ...
      try {
        ...
        if (/e[+-]\d+/.test(output)) {
          displayOutput = output.replace(
            /(.*)e\+*(-*\d+)(\s*\w.*$|$)/,
            '$1 &times; 10<div style="display:inline-block; opacity:0; width:0px;">^</div><sup>$2</sup><span>$3</span>'
          );
        } else {
    ...
    function extractJsCalcOutput(input) {
      ...
      // // For now, don't use javascript ever
      // return {
      // display: null,
      // plain: null
      // };
      ...
      try {
        ...
        if (typeof output === 'number') {
          ...
          if (/e[+-]\d+$/.test(output)) {
            displayOutput = output.replace(
              /(.*)e\+*(-*\d+)/,
              '$1 &times; 10<div style="display:inline-block; opacity:0; width:0px;">^</div><sup>$2</sup>'
            );
          } else {
            displayOutput = output;
          }
    ...
    function extractGoogleCalcOutput(doc) {
      ...
      var docHtml = $doc
        .find('img[src*=calculator-40.gif]')
        .parents('td:eq(0)')
        .siblings()
        .find('*')
        .filter(function () {
      ...
      // If no result try unit conversion boxes
      if (!docHtml) {
      ...
      if (docHtml) {
        ...
        docHtml = docHtml
          ...
          .replace(
            /<sup>/g,
            '<div style="display:inline-block; opacity:0; width:0px;">^</div><sup>'
          )
  ...
  //		<a string>
  resultHtml = (function () {
    function createInputHtml(type, expr) {
      return (
        "<div class='input'><span class='" +
        type +
        "'>" +
        expr +
        '</span></div>'
    ...
    function createInputHtmlEq(type, expr) {
      return (
        "<div class='input'><span class='" +
        type +
        "'>" +
    ...
    function createLinkHtml(queryType, uri) {
      ...
      var $resultLink = $(
        "<div><a target='_blank' class='resultLink'>" +
          (linkText[queryType] || '') +
          '</a></div>'
      );
    ...
    function resultHtml() {
      ...
      var funcPicker = {
        createInputHtml: createInputHtml,
        createInputHtmlEq: createInputHtmlEq,
      };

      var origInputType = 'inputText',
        varSubstInputType = 'inputText';
      ...
      if (result.varName) {
        ...
        resultInstructions = [
          ['origInput', origInputType, 'createInputHtml'],
          ['varSubstInput', varSubstInputType, 'createInputHtml'],
          ['correctedInput', 'inputText', 'createInputHtml'],
        ];
      } else {
        // Instructions for regular input html
        resultInstructions = [
          ['origInput', origInputType, 'createInputHtmlEq'],
          ['varSubstInput', varSubstInputType, 'createInputHtmlEq'],
          ['correctedInput', 'inputText', 'createInputHtmlEq'],
        ];
      }
  ...
  calcVar = (function () {
    ...
    // The most recent variables are kept. When the limit is reached, the old variables get tossed.
    ...
    function createVar(varName, varVal) {
      ...
      if (varName !== '@' && varVal != null && varVal !== 'undefined') {
        // don't save @ or undefined variables
        varMap.size || (varMap.size = 0);
        varMap.maxSize || (varMap.maxSize = 500);
        var varAlreadyExists = !!varMap[varName];

        if (varAlreadyExists) {
          // if variable already exists, remove old links, create new links in the empty space
          if (varMap[varName].prevName) {
            varMap[varMap[varName].prevName].nextName =
              varMap[varName].nextName;
          } else if (varMap[varName].nextName) {
            // we're removing the oldest, set oldest to next oldest
            varMap.oldestVarName = varMap[varName].nextName;
          }
          if (varMap[varName].nextName) {
            varMap[varMap[varName].nextName].prevName =
              varMap[varName].prevName;
          }
        }
        ...
        if (varMap.prevVarName) {
          // make sure there is a previous variable
          varMap[varMap.prevVarName].nextName = varName;
        }

        if (!varAlreadyExists) {
          if (varMap.size === varMap.maxSize) {
            // remove oldest variable if max size reached
            ...
            // if variable name isn't in use already, variable map size gets one bigger
            varMap.size++;
          }
        }
    ...
    // substitute values for calculator variables into an expression
    function substVar(expr) {
      ...
      if (expr) {
        ...
        if (varNameArr) {
          // make sure we have variables other than the previous output "@" variable
        ...
        // put lh var name back if original expr was an assignment
        if (lhName) {
          expr = lhName + ' = ' + expr;
        }
      } else {
  ...
  calcCmd = (function () {
    // -----------------------------------------------------------------------
    var options, // List of stored options
    ...
    function loadOptions() {
      $(options).each(function (i, opt) {
        console.debug(
          ...
          opt,
          '->',
          JSON.parse(localStorage['opt_' + opt] || 'false'),
          '->',
          [defaultOptions[opt]],
          '???',
          defaultOptions
    ...
    function setDefaultOptions() {
      ...
      for (key in defaultOptions) {
        console.debug(
          ...
          key,
          getOption(key)[0],
          typeof getOption(key)[0]
        );
        console.debug(
          'localStorage["opt_' + key + '"]:',
          localStorage['opt_' + key],
          'getOption("' + key + '")[0]',
          getOption(key)[0]
        );
