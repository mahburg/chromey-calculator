...
var UNIT = (function () {
  ...
  var unitDefs = {
    ...
    centimeter: {
      dim: L,
      n: 0.01,
      alias: ['centimeter', 'centimeters', 'centimetre', 'centimetres', 'cm'],
    },
    millimeter: {
      dim: L,
      n: 0.001,
    ...
    kilometer: {
      dim: L,
      n: 1000,
      alias: ['kilometer', 'kilometers', 'kilometre', 'kilometres', 'km'],
    },
    ...
    liter: {
      dim: VOLUME,
      n: 0.001,
      alias: ['liter', 'liters', 'litre', 'litres', 'ltr'],
    },
    milliliter: {
      dim: VOLUME,
      n: 1e-6,
      alias: ['milliliter', 'mililiter', 'mililiters', 'ml'],
    },
    gallon: {
      dim: VOLUME,
      n: cupFactor * 16,
      alias: ['gallon', 'gallons', 'gal', 'gals'],
    },
    quart: {
      dim: VOLUME,
      n: cupFactor * 4,
      alias: ['quart', 'quarts', 'qt', 'qts'],
    },
    pint: {
      dim: VOLUME,
      n: cupFactor * 2,
      alias: ['pint', 'pints', 'pt', 'pts'],
    },
    cup: { dim: VOLUME, n: cupFactor, alias: ['cup', 'cups'] },
    tablespoon: {
      dim: VOLUME,
      n: cupFactor / 16,
      alias: ['tablespoon', 'tablespoons', 'tbsp', 'tblsp'],
    },
    teaspoon: {
      dim: VOLUME,
      n: cupFactor / 48,
      alias: ['teaspoon', 'teaspoons', 'tsp'],
    },
    ...
    microsecond: {
      dim: T,
      n: 1e-6,
      alias: ['microsecond', 'microseconds', 'microsec', 'microsecs'],
    },
    millisecond: {
      dim: T,
      n: 1e-3,
    ...
    year: {
      dim: T,
      n: 3600 * 24 * 365.242199,
      alias: ['year', 'years', 'yr', 'yrs'],
    },
    decade: {
      dim: T,
      n: 10 * 3600 * 24 * 365.242199,
      alias: ['decade', 'decades'],
    },
    score: { dim: T, n: 20 * 3600 * 24 * 365.242199, alias: ['score'] },
    century: {
      dim: T,
      n: 100 * 3600 * 24 * 365.242199,
      alias: ['century', 'centuries'],
    },
    millennium: {
      dim: T,
      n: 1000 * 3600 * 24 * 365.242199,
      alias: ['millennium', 'millenium', 'millenia'],
    },
    ...
    milligram: {
      dim: M,
      n: 1e-6,
      alias: ['milligram', 'miligram', 'miligrams', 'mg'],
    },
    ...
    Fahrenheit: {
      ...
      inverseFunc: function (y) {
        return ((y - 273.15) * 9) / 5 + 32;
      },
      alias: ['Fahrenheit', 'Farenheight', 'farenheight', 'F'],
    },
...
(function () {
  ...
  function Func(funcName, arg) {
    // TODO: Remove this functions if we don't end up using it.
...
// Example <opsArr>: 	Don't pass anything. Used for recursive calls. (Defaults to ORDERED_OPS array.)
// Example return: 		[Plus, "1", [Times, [Power, "3", "2"], "5"], [Minus, [Func, "sin", "5"], [Divide, "3", "2"]]]
function splitToNestedOpArr(strArr, opsArr) {
  opsArr || (opsArr = ORDERED_OPS.slice(0));
  var op = opsArr.shift();
...
function parenToArr(str) {
  ...
  if (openParenPos !== -1) {
    for (
      var i = openParenPos + 1, parenCount = 1, len = str.length;
      0 < parenCount && i < len;
      i++
    ) {
